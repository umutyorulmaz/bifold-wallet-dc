//PushNotificationsHelper-initial.ts
//import { Agent, ConnectionRecord, ConnectionType } from '@credo-ts/core'
// eslint-disable-next-line import/no-extraneous-dependencies
import { Agent, ConnectionRecord, ConnectionType } from '@credo-ts/core'
import { PushNotificationsFcmApi } from '@credo-ts/push-notifications'
...
const setDeviceInfo = async (agent: Agent, blankDeviceToken = false): Promise<void> => {
  const token = blankDeviceToken ? '' : await messaging().getToken()
  const devicePlatform = Platform.OS

  const mediator = await getMediatorConnection(agent)
  if (!mediator) {
    agent.config.logger.error('No mediator connection found')
    return
  }

  agent.config.logger.info(`Trying to send device info to mediator with connection [${mediator.id}]`)
  try {
    const pushNotificationsFcmApi = agent.dependencyManager.resolve(PushNotificationsFcmApi)
